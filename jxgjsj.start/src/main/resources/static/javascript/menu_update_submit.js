...
layui.use(['element','form'], function(){
    ...
    var form = layui.form;
    //监听提交
    form.on('submit(menu_update_submit)', function(){
        var action ="/menu_Update";//表单提交URL地址
        var mid = $("input[name='mid']").val(); //表单数据
        var mname = $("input[name='mname']").val(); //表单数据
        var mpath = $("input[name='mpath']").val(); //表单数据
        $.post(action,{'mid':mid,'mname':mname,'mpath':mpath},function(obj){
            if(obj == true){
                layer.msg('修改成功', {
                    icon: 1,//提示的样式
